...
const FilmNavigationButton: React.FC<props> = ({
    ...
    return (
        <TransitionLink href={`films/${film.id}`} className="2xl:text-xl w-full flex flex-col justify-center text-typography-black hover:text-warm-white relative group
                    ease-in-out z-10 transition duration-300 h-fit align-bottom md:h-full overflow-hidden">
            <div className="flex flex-col h-full justify-center gap-y-6 2xl:gap-y-3 p-6">
                <div className="flex flex-row justify-between pr-4 items-center w-full">
                    <div className='gap-x-6 flex justify-between items-center'>
                        <h1 className="font-semibold text-5xl 2xl:text-6xl">{film.title}</h1>
                ...
                <p className="line-clamp-none 2xl:line-clamp-2 overflow-clip">
                    {film.description}
                </p>
            </div>
